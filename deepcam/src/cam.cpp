...
void cam::Camera_Init(Device& device)
{
    cam Cam;
    ...
    result=device.open("2bc5/0401@1/92");
    ...
    while( key!=27 )
    {
        if( (oniDepthStream.readFrame( &oniDepthImg ) == STATUS_OK) && (oniColorStream.readFrame( &oniColorImg ) == STATUS_OK) )
        {
            ...
            coorvert.convertDepthToWorld(oniDepthStream, oniDepthImg.getWidth()/2+100, oniDepthImg.getHeight()/2, src.at<ushort>(240,220), &x, &y, &z);
            cout<<x<<"   "<<y<<"    "<<z<<" right   "<<endl;
            coorvert.convertDepthToWorld(oniDepthStream, oniDepthImg.getWidth()/2, oniDepthImg.getHeight()/2, src.at<ushort>(240,320), &x0, &y0, &z0);
            cout<<x0<<"  "<<y0<<"  "<< z0<<"  middle"<<endl;
            coorvert.convertDepthToWorld(oniDepthStream, oniDepthImg.getWidth()/2-100, oniDepthImg.getHeight()/2, src.at<ushort>(240,420), &x1, &y1, &z1);
            cout<<x1<<"    "<<y1<<"    "<<z1<<"   "<<"  left"<<endl;
